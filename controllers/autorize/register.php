...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmt = $conn->prepare("SELECT id FROM user WHERE name = ? OR gmail = ?");
    ...
    if ($user_id) {
        echo'<div class="message">Користувач з таким ім\'ям або gmail вже існує!</div>';
    } else {
        $stmt->close();
        $stmt = $conn->prepare("INSERT INTO user (name, login, gmail, password) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $username, $login, $gmail, $password);
        if ($stmt->execute()) {
            header("Location: /view/Main");
        }else{
            echo'<div class="message">ПОМИЛКА РЕЄСТРАЦІЇ</div>';
        }
    }
